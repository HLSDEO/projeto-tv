...
export default function AdminDashboard() {
  ...
  const handleUpdateMedia = async (m, updates) => {
    try {
      ...
      await api.put(`/api/media/${m._key}`, {
  ...
  const toggleNews = async () => {
    try {
      await api.put('/api/settings', { news_enabled: !settings.news_enabled });
      fetchData();
    } catch (err) {
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="bg-zinc-800 p-6 rounded-xl border border-zinc-700">
          ...
          <div className="space-y-4">
            {media.map((m, idx) => (
              <div key={m._key} className={`flex items-center gap-4 p-4 rounded-lg border ${m.active ? 'bg-zinc-900 border-zinc-700' : 'bg-zinc-900/50 border-zinc-800 opacity-75'}`}>
